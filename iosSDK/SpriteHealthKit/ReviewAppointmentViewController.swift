...
class ReviewAppointmentViewController: UIViewController, SFSafariViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        cmObj.getDeveloperAccount (){ [self] (error, applicationAccount) in
             ...
             if let error = error {
                 print(error)
                    ...
                    if applicationAccount?.name != nil {
                            companyDetail["name"] = applicationAccount?.name!
                    }
                    else {
                        companyDetail["name"] = ""
                    }
                if applicationAccount?.privacyPolicyLink != nil {
                        companyDetail["privacyPolicyLink"] = applicationAccount?.privacyPolicyLink!
                }
                else {
                    companyDetail["privacyPolicyLink"] = ""
                }
                if applicationAccount?.termsOfUseLink != nil {
                        companyDetail["termsOfUseLink"] = applicationAccount?.termsOfUseLink!
                }
                else {
                    companyDetail["termsOfUseLink"] = ""
                }
                
                if applicationAccount?.consentToCareLink != nil {
                        companyDetail["consentToCareLink"] = applicationAccount?.consentToCareLink!
                }
                else {
    ...
    @IBAction func gobackTomain(_ sender: Any) {
        ...
        let requestParameters: [String: Any] = [
            ...
            "reasonName" : String(InfoArr!["reason"]!)
            ]
       ...
       // print(urlString)
        ...
        let shObject = SpriteHealthClient.getInstance()   // Constructor
            shObject.createAppointment(formPost: requestParameters){ (error, result) in
                if let error = error {
                print(error)
                } else {
                // do stuff with the result
                //print(result)
                    ...
                    if result?.appointment != nil {
                        ...
                        let appointmentId = result?.appointment?.id
                            DispatchQueue.main.async {
                                ...
                                DispatchQueue.main.async {
                                    var urlString: String =  SpriteHealthClient.webClientRoot + "/globals/appointments/<appointmentId>?" + "wlb=true%3Bimplicit%3B" + self.organizationId! + "#access_token=<access_token>"
