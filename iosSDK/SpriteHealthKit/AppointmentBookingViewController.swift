...
class AppointmentBookingViewController: UIViewController,UITableViewDelegate, UITableViewDataSource  {
    ...
    override func viewDidLoad() {
        ...
        var urlQueryParams = ["serviceId" :  serviceId!, "vendorUserId" :  specialistId!, "week" : 3, "startDateTime" : startDateTime, "currentTime" : currentTime] as [String : Any]
        //print(urlQueryParams)
        //, "startDateTime" : startDateTime, "currentTime" : currentTime
        cmObj.getSpecialistAvailability(queryParams: urlQueryParams){ (error, result) in
            if let error = error {
                print(error)
            } else {
            // do stuff with the result
                ...
                self.displayDetails(specialistAvailabilities: result)
                   ...
                   //print(json);
                    ...
                    DispatchQueue.main.async { [self] in
                       ...
                       // print(requestParameters)
                        cmObj.getServiceNetworkCoverage(formPost: requestParameters)
                        { (error, result) in
                            if let error = error {
                                print(error)
                                myActivityIndicator.stopAnimating()
                                ...
                                // do stuff with the result
                                    ...
                                    DispatchQueue.main.async { [self] in
                                        ...
                                        if result?.groupedTotalPatientAmount != nil{
                                            if let insured = result?.groupedTotalPatientAmount!["Insured"] {
        ...
        urlQueryParams = ["specialities" : "26", "slim": "HIGH"]
        cmObj.getReasons(queryParams: urlQueryParams){ (error, result) in
            // do stuff with the result
            if let error = error {
                print(error)
            } else {
    ...
    func pushReasons(json:[Reason]?) {
        if(json != nil)
        {
            if json!.count>0 {
                for reason in json!
                {
                    dataSource.append(reason.name)
                }
            }
        }
    }
    func displayDetails(specialistAvailabilities: SpecialistAvailability?) {
       
        
        var count = 0;
        if specialistAvailabilities?.freeTimePeriods != nil
            {
            
            for freeTimePeriod in (specialistAvailabilities?.freeTimePeriods)!
            {
                
                let startTime =  freeTimePeriod.startTime!
                let endTime = freeTimePeriod.endTime!
                //print(startTime)
                //print(endTime)
    ...
    @objc func scrollButtonAction(sender: UIButton) {
        ...
        specialistArr["reason"] = selectedReason
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        selectedReason = dataSource[indexPath.row]
